...
// потрібні для мінімального hardening Express-застосунку
...
// demoAuth middleware
// імітація авторизації через заголовок X-Demo-UserId
// використовується для демонстрації захисту від IDOR

function demoAuth(req, res, next) {
    const userId = Number(req.header("X-Demo-UserId"));

    // якщо заголовок відсутній або некоректний
    if (!Number.isInteger(userId) || userId <= 0) {
        return fail(res, "Необхідний заголовок X-Demo-UserId", 401);
    }

    // додаємо userId у request
    req.user = {
        id: userId
    };
...
// пошук користувачів (захищено від SQL Injection через параметризований запит)
app.get("/api/users/search", async (req, res) => {
    try {
        const { role, limit = 5 } = req.query;

        // limit теж перевіряємо, бо це вхідні дані від користувача
        const numericLimit = Number(limit);

        if (!Number.isInteger(numericLimit) || numericLimit < 1 || numericLimit > 50) {
            return fail(res, "Некоректний limit", 400);
        }
        ...
        // role більше не вставляється напряму в SQL
...
app.get("/api/v1/passes/:id", demoAuth, async (req, res) => {
    try {
...
app.post("/api/v1/passes", async (req, res) => {
    try {
        ...
        let safeReasonId: number | null = null;
...
// оновити пропуск
app.put("/api/v1/passes/:id", demoAuth, async (req, res) => {
    try {
        const passId = Number(req.params.id);
        const { userId, reasonId, date, status } = req.body;
        ...
        const result = await run(`
            ...
            SET userId = ?,
                ...
                status = ?
            WHERE id = ? AND userId = ?;
        `, [numericUserId, safeReasonId, safeDate, safeStatus, passId, req.user.id]);
...
// видалити пропуск
app.delete("/api/v1/passes/:id", demoAuth, async (req, res) => {
    try {
        const passId = Number(req.params.id);
...
app.use((req, res) => {
    return fail(res, "Маршрут не знайдено", 404);
});
